    ...
    <BasicForm @register="registerForm">
      <template #picDrawer="{ model, field }">
        <template v-if="model[field] > 0">
          <Image :src="getImageUrlById(model[field])" :preview="false" :width="80" />
...
<script lang="ts">
  ...
  import { formSchema } from './diningTable.data';
  ...
  export default defineComponent({
    ...
    setup(_, { emit }) {
      ...
      const getTitle = computed(() => (!unref(isUpdate) ? '新增餐桌' : '编辑餐桌'));
