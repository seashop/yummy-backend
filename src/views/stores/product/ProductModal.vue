<template>
  ...
  >
    <BasicForm @register="registerForm" @field-value-change="fieldValueChange">
      ...
      <template #sku="{}">
        <FormItemRest>
          <Sku :rdata="state.rdata" @result="handleSkuResult" />
        </FormItemRest>
      </template>
    </BasicForm>
  </BasicModal>
...
<script lang="ts" setup>
  ...
  const emit = defineEmits(['success', 'register', 'back']);
  ...
  const [registerForm, { resetFields, resetSchema, getFieldsValue, setFieldsValue, validate }] =
    useForm({
      labelWidth: 90,
