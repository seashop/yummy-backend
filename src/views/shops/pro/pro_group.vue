								...
								<el-table :data="category" border style="width: 100%" :default-sort="{prop: 'sort', order: 'descending'}">
									...
									<el-table-column  label="分类名字">
										<template slot-scope="scope">
											...
											<span v-if="scope.row.pid > 0">&emsp;&emsp;|——</span>{{scope.row.category_name}}
										</template>
									</el-table-column>
...
<script>
	...
	import AddCategory from '../components/addGroup.vue'
	import {
		Api_url
		...
		methods: {
			...
			del(id, index) {
				...
				this.$confirm('是否删除?', '提示', {
					...
					this.http.put_show('category/admin/del_category?id='+id).then((res) => {
						if (res.data == 1) {
							that.$message({
								showClose: true,
								message: res.msg,
