					...
					<div class="article">
													...
													<div style="display: flex; width:700px ; flex-wrap: wrap;">
														<template v-for="(item,index) of img_list">
															<div class="picA" v-if="img_list.length > 0">
																<img class="img" v-if="item&&item.full_url" :src="item.full_url">
						...
						<template v-if="tab_nav == 1">
							<el-table :data="list" border style="width: 100%">
								...
								<el-table-column label="文章图片" width="200%">
									<template slot-scope="scope">
											<template v-if="scope.row.img">
												<img style="height: 100px;" :src="scope.row.img.full_url" />
											</template>
								...
								<el-table-column prop="is_hidden" label="隐藏">
									<template slot-scope="scope">
		...
		methods: {
			...
			edit_get(id) {
				...
				this.http.get_show("article/get_one_article?id=" + id).then(res => {
					...
					if(res.data.img && res.data.img.id) {
						that.form.image = res.data.img.id;
					}
					if (res.data.type == 2) {
