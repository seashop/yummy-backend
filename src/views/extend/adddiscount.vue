		...
		<el-container>
			...
			<el-main class="main">
				<el-breadcrumb separator-class="el-icon-arrow-right">
					...
					<el-breadcrumb-item>新增满减</el-breadcrumb-item>
				</el-breadcrumb>
				<div class="H10"></div>
				<div class="add">
					<div class="add_btn">
					...
					<el-tabs type="border-card" style="margin-bottom: 20px" v-model="active_name">
						<el-tab-pane label="第一步：选择商品" name="one">
							...
							<div class="quan">
								...
								<div class="quan_r">共{{tableData.length}}条，每页{{size}}条</div>
							</div>
						</el-tab-pane>
						<el-tab-pane label="第二步：设置满减" name="two" v-if="is_two == 1">
							<div class="choose">
								<el-table :data="discount_list" style="width: 100%;" :row-class-name="tableRowClassName">
									...
									<el-table-column prop="" label="">
										<template slot-scope="scope" v-if="form.reduce">
											减价：满{{form.full}}减{{form.reduce}}元 {{ tableData[scope.row] }}
	...
	export default {
		...
		methods: {
			...
			onSubmit() {
				console.log(this.form)
				let arr = []
				for (let k in this.form.goods_json) {
					let v = this.form.goods_json[k]
					arr.push(v.goods_id)
				}
				let data = {
					name: this.form.name,
